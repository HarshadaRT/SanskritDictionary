...
HocrProofreader.prototype.renderPage = function (pageNode) {
    ...
    this.layoutImage.setAttributeNS('http://www.w3.org/1999/xlink', 'href', this.hocrBaseUrl + this.config.image);
...
HocrProofreader.prototype.renderNodesRecursive = function (node, options, parentRectsNode) {
    ...
    if (className) {  
		...
		console.log("========",options);
		//console.log("============",options.baselineBbox[0]);
		//console.log("============",options.baselineBbox[1]);
		//console.log("============",options.baselineBbox[2]);
		//console.log("============",options.baselineBbox[3]);
        if (options.bbox) {
            if (className === 'ocrx_word' && options.baselineBbox) {
                var word = node.textContent;
				console.log("ocrx_word :",options);
				
				var scan_res = 1;
				if(typeof options.scan_res !== 'undefined')
					scan_res = options.scan_res[1];
                ...
                var textNode = Util.createSvgElem('text', {
                    ...
                    'font-size': options.x_fsize * scan_res/ 72, // 1 pt = 1/72 inch
